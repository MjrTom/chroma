...
impl Posthog {
    ...
    async fn add_to_buffer(&self, event: Box<dyn ProductTelemetryEvent + Send + Sync>) {
        ...
        let mut buffer_guard = self.event_buffer.lock().await;
        if buffer_guard.len() >= MAX_BUFFER_SIZE {
            self.flush_buffer(&mut buffer_guard).await;
        }
        buffer_guard.push(posthog_event);
    }

    async fn flush_buffer(&self, buffer: &mut Vec<Event>) {
        let events_to_send = buffer.drain(..).collect::<Vec<_>>();
        if let Err(e) = self.client.capture_batch(events_to_send).await {
...
impl TelemetryClient for Posthog {
    async fn capture(&mut self, event: Box<dyn ProductTelemetryEvent + Send + Sync>) {
        ...
        if should_capture_directly {
            let mut seen_types = self.seen_event_types.write().await;
            seen_types.insert(batch_key.clone());
            drop(seen_types); // Explicitly drop the write lock before awaiting
            ...
            match existing_event.batch(event) {
                Ok(batched_event) => {
                    if batched_event.batch_size() >= batched_event.max_batch_size() {
                        drop(batched);
                        self.add_to_buffer(batched_event).await;
                    } else {
